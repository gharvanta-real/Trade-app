...
export const initMarketWebSocket = () => {
  ...
  setInterval(() => {
    ...
    Object.keys(store.symbols).forEach(key => {
...
export const placeRealOrder = async (params: {
  ...
  if (store.paperTradeMode) {
    ...
    return { success: !!newId, message: newId ? 'Paper order placed' : 'Insufficient margin', orderId: newId || undefined };
  }
...
export const searchInstruments = async (query: string): Promise<any[]> => {
  ...
  // Fallback to local mock data matching user request
  const mockInstruments = [
    { name: 'SENSEX', exchange: 'BSE', type: 'Index', lot_size: 10 },
    { name: 'NIFTY 50', exchange: 'NSE', type: 'Index', lot_size: 50 },
    { name: 'BANKNIFTY', exchange: 'NSE', type: 'Index', lot_size: 15 },
    { name: 'FINNIFTY', exchange: 'NSE', type: 'Index', lot_size: 40 },
    { name: 'RELIANCE', exchange: 'NSE', type: 'Equity', lot_size: 1 },
    { name: 'HDFCBANK', exchange: 'NSE', type: 'Equity', lot_size: 1 },
    { name: 'TCS', exchange: 'NSE', type: 'Equity', lot_size: 1 },
    { name: 'INFY', exchange: 'NSE', type: 'Equity', lot_size: 1 },
    { name: 'ICICIBANK', exchange: 'NSE', type: 'Equity', lot_size: 1 },
    { name: 'KOTAKBANK', exchange: 'NSE', type: 'Equity', lot_size: 1 },
    { name: 'SBIN', exchange: 'NSE', type: 'Equity', lot_size: 1 },
    { name: 'AXISBANK', exchange: 'NSE', type: 'Equity', lot_size: 1 },
    { name: 'STATE BANK OF INDIA', exchange: 'NSE', type: 'Equity', lot_size: 1 },
    { name: 'NIFTY 23 MAY 24500 CE', exchange: 'NSE', type: 'Option', lot_size: 50 },
    { name: 'NIFTY 23 MAY 24500 PE', exchange: 'NSE', type: 'Option', lot_size: 50 },
    { name: 'BANKNIFTY 23 MAY 48500 CE', exchange: 'NSE', type: 'Option', lot_size: 15 },
  ];

  if (!qClean) return mockInstruments.slice(0, 8);
  return mockInstruments.filter(item => {
    const name = item.name.toUpperCase();
    if (name.includes(qClean) || name.replace(/\s+/g, '').includes(qClean)) {
      return true;
    }
    const words = name.split(/\s+/);
    if (words.length > 1) {
      const initials = words.map(w => w[0]).join('');
      const initialsNoOf = words.filter(w => w !== 'OF').map(w => w[0]).join('');
      if (initials === qClean || initialsNoOf === qClean) {
        return true;
      }
    }
    return false;
...
// 19. WATCHLIST
const mockPriceList: Record<string, number> = {
  'SBIN': 836.45,
  'STATE BANK OF INDIA': 836.45,
  'AXISBANK': 1317.30,
  'KOTAKBANK': 1756.90,
  'SENSEX': 80234.50,
  'NIFTY 23 MAY 24500 CE': 243.65,
  'NIFTY 23 MAY 24500 PE': 185.10,
  'BANKNIFTY 23 MAY 48500 CE': 356.70,
};
...
export const addWatchlistItem = (symbol: string) => {
  ...
  if (!store.symbols[clean]) {
    const basePrice = mockPriceList[clean] || Math.round(100 + Math.random() * 2000);
    setStore('symbols', clean, {
      name: clean,
      price: basePrice,
      change: 0,
      pct: 0,
      up: true,
      type: clean.includes('CE') || clean.includes('PE') ? 'Option' : clean.includes('SENSEX') || clean.includes('NIFTY') || clean.includes('BANK') || clean.includes('FIN') ? 'Index' : 'Stock',
      open: basePrice * 0.99,
      high: basePrice * 1.01,
      low: basePrice * 0.985,
      prevClose: basePrice,
      volume: '10.5M',
      signal: Math.random() > 0.5 ? 'Strong Bullish' : 'Neutral'
    });
  }
